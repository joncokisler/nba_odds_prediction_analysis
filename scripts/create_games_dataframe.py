...
def create_games_dataframe():
    ...
    games_sorted = all_games.sort_values(by='GAME_DATE').drop(['TEAM_ID'], axis=1)
    exclude_cols = ['TEAM_NAME', 'GAME_DATE', 'WL', 'GAME_ID', 'SEASON_ID']
    ...
    for team in teams:
        ...
        team_ma_A_list.append(
            sorted_team_vals[cols_to_roll].rolling(5, min_periods=1).mean(numeric_only=True).join(re_add_cols))
